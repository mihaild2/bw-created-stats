document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadUsers(forceFetch = false) {
    try {
      ...
      const response = await fetch(
        "https://app.browswave.com/iui-api/nats/get-activity-filters",
        {
          method: "POST",
          headers: {
      ...
      if (json.data && Array.isArray(json.data.users)) {
        ...
        userListEl.innerHTML =
          '<div style="padding:10px; color:#ff5252">Invalid data format received.</div>';
      }
    } catch (error) {
      console.error(error);
      userListEl.innerHTML =
        '<div style="padding:10px; color:#ff5252">Error loading users. Check connection.</div>';
    }
  }
  ...
  function saveSelection() {
    const checkboxes = userListEl.querySelectorAll(
  ...
  function updateSelectedCount() {
    const count = userListEl.querySelectorAll(
      'input[type="checkbox"]:checked'
    ).length;
  ...
  async function startFiltration() {
    ...
    const selectedCheckboxes = Array.from(
      userListEl.querySelectorAll('input[type="checkbox"]:checked')
    );
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const count =
            data.data && typeof data.data.count !== "undefined"
              ? data.data.count
          ...
          const link = `https://app.browswave.com/i/norm/clusters?filter=${encodeURIComponent(
            linkFilter
          )}&pageIndex=1&pageSize=100&sortBy=[]`;
  ...
  function updateSortIcons() {
    const targetEle = document.querySelectorAll("th")[1];
    targetEle.classList.remove("sort-active", "asc", "desc");
    targetEle.querySelector(".arrow").textContent = "▼"; // reset
    const activeTh = document.querySelector(
      `th[data-sort="${currentSort.key}"]`
    );
    if (activeTh) {
      activeTh.classList.add("sort-active", currentSort.dir);
      activeTh.querySelector(".arrow").textContent =
        currentSort.dir === "asc" ? "▲" : "▼";
    }
  }
  ...
  function renderTable() {
    ...
    results.forEach((row) => {
      ...
      let diffHtml = "";
      if (row.diff > 0) {
        diffHtml = `<span class="diff-tag diff-pos">+${row.diff.toFixed(
          1
        )}%</span>`;
      } else if (row.diff < 0) {
        diffHtml = `<span class="diff-tag diff-neg">${row.diff.toFixed(
          1
        )}%</span>`;
      } else {
